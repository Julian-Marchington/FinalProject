...
public class MultiSimpleGazeDetector: MonoBehaviour
{
    [Header("Models")]
    ...
    public SimpleGazeVisualizer gazeVisualizer;    // Gaze arrows visualizer
    ...
    [Tooltip("Skip tiny boxes to save time.")]
    public float minFaceSize = 80f;
    ...
    // global bias (press C to zero to your head/camera)
    ...
    void Update()
    {
        ...
        try
        {
            ...
            if (faces.Count == 0)
            {
                ...
                gazeVisualizer?.HideUnusedArrows(0);
                return;
            }
            _noFaceFrames = 0;
            ...
            {
                var face = faces[i];
                if (Mathf.Min(face.width, face.height) < minFaceSize) continue;

                var gazeAngles = RunGazeOnFace(_cols640, face);

                // visualizer (one arrow per id)
                gazeVisualizer?.UpdateGazeDirectionForId(i, face, gazeAngles.yaw, gazeAngles.pitch);

                // manual calibration on key (global)
                ...
                // auto-calibration once when a face is large enough
                if (!_didAutoCalib)
                {
                    float side = Mathf.Max(face.width, face.height);
                    if (side > 160f)
                    {
                        _yawBias = gazeAngles.yaw;
                        _pitchBias = gazeAngles.pitch;
                ...
                // Per-person log
                bool looking = IsGazeAtCamera(gazeAngles.yaw, gazeAngles.pitch);
                if (looking) anyoneLooking = true;
                Debug.Log($"[GAZE] Person #{i}: {(looking ? "LOOKING" : "not looking")}");
            }

            gazeVisualizer?.HideUnusedArrows(faces.Count);

            if (anyoneLooking)
                Debug.Log("[GAZE] Someone is looking at the camera!");
        }
    ...
    {
        // 1) Bias-correct (personal/global zero)
    ...
    List<Rect> DetectFaces(int maxK)
    {
        ...
        // Create input tensor (keep vertical flip your YOLO expects)
        ...
        using (var output = _faceWorker.PeekOutput() as Unity.InferenceEngine.Tensor<float>)
        {
            ...
            foreach (var d in kept)
            {
                ...
                // UNFLIP rectangle back to readback pixel coords

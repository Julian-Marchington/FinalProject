        ...
        public static bool IsSupported
        {
            get
            {
                if (!s_isSupported.HasValue)
                {
                    ...
                    var horizonVersion = HorizonOSVersion;
                    
                    // Log detailed information for debugging
                    Debug.Log($"PCA: Checking passthrough camera support:");
                    Debug.Log($"PCA: - Headset type: {headset}");
                    Debug.Log($"PCA: - Horizon OS version: {(horizonVersion.HasValue ? horizonVersion.Value.ToString() : "Unknown")}");
                    Debug.Log($"PCA: - Minimum required version: {MINSUPPORTOSVERSION}");
                    
                    bool isSupported = (headset == OVRPlugin.SystemHeadset.Meta_Quest_3 ||
                                      headset == OVRPlugin.SystemHeadset.Meta_Quest_3S) &&
                                     (!horizonVersion.HasValue || horizonVersion >= MINSUPPORTOSVERSION);
                    
                    // Special handling for Meta Link scenarios
                    if (headset == OVRPlugin.SystemHeadset.Meta_Link_Quest_3)
                    {
                        Debug.LogWarning("PCA: Meta Link detected - passthrough camera access limited. This is expected behavior when running through Meta Link.");
                        Debug.LogWarning("PCA: For full passthrough camera functionality, run directly on Quest 3/3S headset.");
                    }
                    
                    s_isSupported = isSupported;
                    Debug.Log($"PCA: Passthrough camera support: {(isSupported ? "YES" : "NO")}");
                }

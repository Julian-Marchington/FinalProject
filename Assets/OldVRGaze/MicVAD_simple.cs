...
/// Simple on-device VAD using adaptive RMS energy & hysteresis (no external services).
/// Emits: IsSpeech, SilenceMs, and C# events when speech starts/ends.
public class MicVAD_simple : MonoBehaviour
{
    [Header("Mic")]
    public string deviceName = null;       // null = default mic
    public int sampleRate = 16000;         // Quest is fine with 16k; Unity resamples if needed
    public int loopSeconds = 10;           // length of the rolling clip
    public int frameMs = 20;               // analysis window (20ms)
    ...
    public float QuietMs { get; private set; }    // ms of continuous raw quiet below quietDbThreshold
    public float SilenceMs { get; private set; }    // ms since last speech
    public float LevelDb { get; private set; }      // instantaneous dBFS
    public float NoiseFloorDb { get; private set; } // adaptive floor
    ...
    void Start()
    {
        ...
        // If you left deviceName null, Unity uses the OS default input device.
    ...
    System.Collections.IEnumerator WaitMicThenInit()
    {
        ...
        // initialize noise floor to a conservative value
        NoiseFloorDb = -50f;
        SilenceMs = 0f;
        _hangTimer = 0f;
        LogRow("INIT");
    ...
    void Update()
    {
        ...
        // Read a frame from the rolling clip (ring buffer style)
        ...
        {
            float s = buf[i]; // take channel 0
            sum += s * s;
        }
        double rms = System.Math.Sqrt(sum / (buf.Length / _channels + 1e-9));
        LevelDb = (float)(20.0 * System.Math.Log10(rms + 1e-9)); // dBFS ~ [-inf, 0]

        // --- Simple volume-based quiet timer ---
        if (LevelDb <= quietDbThreshold)
            QuietMs += frameMs;
        else
            QuietMs = 0f;

        // Update noise floor only when not in speech (slow EMA)
        if (!IsSpeech)
        {
            // clamp max floor so it doesn't drift too high
        ...
        if (!IsSpeech)
        {
            // Enter speech if we rise above the higher threshold
        ...
        {
            // Stay in speech while above hold threshold or hangover timer running

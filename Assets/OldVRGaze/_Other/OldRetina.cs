...
public class OldRunner : MonoBehaviour
{
    [Header("Model (RetinaFace ONNX)")]
    public Unity.InferenceEngine.ModelAsset retinaModel;
    public Unity.InferenceEngine.BackendType backend = Unity.InferenceEngine.BackendType.CPU;

    [Header("Video Source")]
    public PCAFeedBinder pcaFeed; // Quest passthrough binder
    public bool useQuestPCA = true;
    [Header("Webcam (PC)")]
    public int requestWidth = 1280, requestHeight = 720, requestFPS = 30;

    [Header("Preprocess (RetinaFace standard)")]
    public int inputSize = 640;                   // 640x640
    public bool inputIsBGR = true;                // RetinaFace expects BGR
    public bool subtractBGRMean = true;           // subtract [104,117,123]
    public Vector3 bgrMean = new Vector3(104, 117, 123);

    [Header("Priors (RetinaFace default)")]
    public int[] steps = new int[] { 8, 16, 32 };
    public Vector2Int[] minSizes = new Vector2Int[]{
        new Vector2Int(16,32), new Vector2Int(64,128), new Vector2Int(256,512)
    };
    public Vector2 variance = new Vector2(0.1f, 0.2f); // center/size

    [Header("Thresholds")]
    [Range(0.05f, 0.99f)] public float confThreshold = 0.6f;
    [Range(0.0f, 1.0f)] public float nmsIou = 0.4f;
    public float minFaceSidePx = 100f;
    public int maxFaces = 10;

    [Header("Look-at (landmark symmetry)")]
    public float maxNoseAsym = 0.12f; // legacy (unused by decision)
    public float maxEyeTilt = 0.05f; // legacy (unused by decision)

    [Header("Look-at (frontal thresholds, roll-invariant)")]
    [Tooltip("Max allowable head roll (deg) to still count as frontal")]
    public float maxRollDeg = 18f; // still used by IsFrontal's early reject
    [Tooltip("Max horizontal nose offset (fraction of eye spacing) (legacy)")]
    public float maxNoseAsymX = 0.40f;
    [Tooltip("If eye spacing is < this fraction of face WIDTH, skip look test")]
    public float minInterFrac = 0.22f;
    [Tooltip("Absolute minimum eye spacing (pixels) to trust landmarks")]
    public float minInterAbsPx = 20f;

    [Header("Audio & Debounce")]
    public AudioSource audioSource;
    public AudioClip lookAtClip;
    public float enterCooldown = 0.8f;
    public bool beepOnLook = false;

    [Header("Look Stability (anti-flicker)")]
    [Tooltip("Require this long to qualify as LOOKING")]
    public float enterLookDwellSec = 0.45f;
    [Tooltip("Require this long to qualify as NOT LOOKING")]
    public float exitLookDwellSec = 0.25f;
    [Tooltip("Stricter to ENTER looking, looser to EXIT (hysteresis)")]
    public float noseAsymEnter = 0.35f;
    public float noseAsymExit = 0.50f;
    [Tooltip("Stricter to ENTER looking, looser to EXIT (hysteresis)")]
    public float rollEnterDeg = 15f;
    public float rollExitDeg = 25f;
    [Tooltip("How similar rects must be to keep same face track")]
    [Range(0f, 1f)] public float iouTrackThreshold = 0.20f; // stickier
    ...
    public string sessionIdOverride = ""; // optional
    ...
    // ---- runtime
    Unity.InferenceEngine.Worker _wk;
    Unity.InferenceEngine.Model _model;

    WebCamTexture _pcWebcam;
    RenderTexture _rt;
    Texture2D _readback;
    Color32[] _cols;
    bool _haveCols;
    ...
    Rect _lastBestRect;
    public float dropoutGraceSec = 0.55f; // bridges tiny holes
    ...
    struct Det
    {
        public Rect rect;
        public float score;
        public Vector2[] landmarks; // 5 points
    }
    List<Det> _lastDets = new List<Det>(16);

    public struct FaceObs
    {
        public Rect rect; public float side; public float score;
        public Vector2 LE, RE, N;
        public bool isLooking;
    }
    public IReadOnlyList<FaceObs> FacesObs => _facesObs;
    readonly List<FaceObs> _facesObs = new List<FaceObs>(8);

    // Discovered output names (from the imported model)
    string _outLoc, _outConf, _outLandm;

    // ===== Enhanced Logging (TXT + CSV) =====
    string _logPathTxt, _logPathCsv;
    StreamWriter _logTxt, _logCsv;
    System.Diagnostics.Stopwatch _sw = new System.Diagnostics.Stopwatch();
    string _sessionId;

    // --- Best-face stability state (anti-flicker) ---
    Rect _bestTrackRect;
    bool _bestLookStable;
    float _bestEnterTimer, _bestExitTimer;
    float _bestLastSeenTime;

    // --- Smoothing state for best-face landmarks ---
    Vector2 _LEs, _REs, _Ns;
    bool _haveSmooth = false;
    [Range(0f, 1f)] public float smoothAlpha = 0.5f; // 0=no smoothing, 1=instant

    // --- Edge-triggered beep state ---
    bool _prevBestLook = false;
    bool _beepedForThisTrack = false;
    float _stableFalseTimer = 0f;
    [Tooltip("How long 'not-looking' must persist before another beep is allowed")]
